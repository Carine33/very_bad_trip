...
/*			if(strlen($post['username']) < 5){
	...
	/**
	 * Mise à jour de l'utilisateur par un admin
	 */
	...
	/**
	 * Mise à jour de l'utilisateur par l'utilisateur
	 */
	public function updateSigninUser(){
		...
		} // fin !empty($_FILES) && isset($_FILES['avatar']) && $_FILES['avatar']['error'] == UPLOAD_ERR_OK
		...
		if(!empty($_POST)){
			...
			if(count($errors) === 0){
				...
				if($usersModel->update($data, $post['id'])){
					// Ici l'insertion en base est effectuée
					$authModel->refreshUser(); // Raffraichit les données en session de l'user
					$currentUser = $this->getUser(); // Contient les données de l'utilisateur
